...
class TypeAPipeline:
    ...
    def report_progress(self, curr, total, s, f):
        try:
            with open(".progress.json", "w") as file: json.dump({"current": curr, "total": total, "success": s, "fail": f, "status": f"Rate limit hit - waiting {int(tracxn_limiter.current_wait_sec//60)} mins" if tracxn_limiter.current_wait_sec > 60 else ("Rate limit hit - waiting <1 min" if tracxn_limiter.current_wait_sec > 0 else "running")}, file)
        except: pass
