...
app = FastAPI(title="SR Publishing Type B API")
...
async def run_pipeline_task(request: RunRequest):
    ...
    try:
        ...
        logger.info(f"Starting Type B pipeline: {' '.join(cmd)}")
        ...
        if state.get("is_stopping"):
            logger.info("Type B pipeline stopped gracefully")
            state["status"] = "stopped"
        elif process.returncode == 0:
            logger.info("Type B pipeline completed successfully")
            state["status"] = "succeeded"
        else:
            logger.error(f"Type B pipeline exited with code {process.returncode}")
            state["status"] = "failed"
...
@app.get("/typeb/status", dependencies=[Depends(verify_token)])
async def get_status():
    ...
    return {
        ...
        "workerName": f"{os.environ.get('WORKER_IDENTITY', os.uname().nodename)}-TypeB-Pipeline"
    }

@app.get("/typeb/health", dependencies=[Depends(verify_token)])
async def health_check():
    return {"status": "ok", "timestamp": utc_now()}

@app.post("/typeb/start", dependencies=[Depends(verify_token)])
...
@app.post("/typeb/cancel", dependencies=[Depends(verify_token)])
