        ...
        async with aiohttp.ClientSession() as session:
            ...
            if self.mode == "phase2":
                ...
                try:
                    ...
                    tasks = [asyncio.create_task(self.domain_worker(work_queue, result_queue, browser_mgr, session, prompts, paths, f_ids, bm_paths, bm_map, f_defs, h_map, cache_manager)) for _ in range(CONFIG["MAX_WORKERS"])]
                    await work_queue.join()
                    for t in tasks: t.cancel()
                finally:
