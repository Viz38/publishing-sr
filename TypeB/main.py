...
async def process_domain_stage1(browser, session, row, prompts, paths, f_ids, bm_paths, bm_map, f_defs, h_map, cache_manager) -> Dict:
    ...
    # Check for parked (use body for both args — html may not exist for manual content)
    parked, kw = is_parked_domain(body, body)
    if parked:
    ...
    if len(parts_p1) == 2:
        ...
        res_p1_obj = await call_gemini_api(session, p1, gemini_limiter, api_key=CONFIG["GEMINI_API_KEY"])
    ...
    return {
        ...
        "feed_id": f_id, "body_len": len(body), "scraper_used": scraper_used,
...
class TypeBPipeline:
    ...
    async def _stop_monitor(self, w_q):
        ...
        while not (self.stop_requested or os.path.exists(STOP_FILE)):
    ...
    async def run(self):
        ...
        async with aiohttp.ClientSession() as session:
            ...
            writer_task = asyncio.create_task(self.sheet_writer(result_queue, ws, len(data_rows), gc, "TypeB", tracxn_queue))
            ...
            if self.mode == "phase2":
                tasks = [asyncio.create_task(self.domain_worker(work_queue, result_queue, tracxn_queue, None, session, prompts, paths, f_ids, bm_paths, bm_map, f_defs, h_map, cache_manager)) for _ in range(CONFIG["MAX_WORKERS"])]
                ...
                try:
                    ...
                    tasks = [asyncio.create_task(self.domain_worker(work_queue, result_queue, tracxn_queue, browser_mgr, session, prompts, paths, f_ids, bm_paths, bm_map, f_defs, h_map, cache_manager)) for _ in range(CONFIG["MAX_WORKERS"])]
                    await work_queue.join()
                    for t in tasks: t.cancel()
                finally:
    ...
    async def heartbeat_reporter(self, t_q):
        ...
        while True:
            try:
                ...
                current_data["is_stopping"] = self.stop_requested or os.path.exists(STOP_FILE)
                
                tmp_file = os.path.join(BASE_DIR, f".progress.json.tmp.{os.getpid()}")
    ...
    async def domain_worker(self, w_q, r_q, t_q, browser, session, prompts, paths, f_ids, bm_paths, bm_map, f_defs, h_map, cache_manager):
        monitor = SystemHealthMonitor()
        import random
        # Jitter start to prevent CPU storm
        await asyncio.sleep(random.uniform(0.5, 3.0))
        while True:
            if self.stop_requested or os.path.exists(STOP_FILE):
            ...
            try:
                ...
                if self.mode == "phase2":
                    ...
                    try:
                        res = await asyncio.wait_for(
                            process_domain_stage1(browser, session, row, prompts, paths, f_ids, bm_paths, bm_map, f_defs, h_map, cache_manager),
                            timeout=75.0
                        )
                    except asyncio.TimeoutError:
                ...
                if self.mode != "phase2":
                    if is_success:
                        ...
                        raw_data_col = h_map.get("raw_data_col", "X")
                ...
                if self.mode != "phase1":
                    # Decoupled: hand off to async Tracxn queue so scraping & LLM continue uninterrupted
    ...
    def report_progress(self, curr, total, s, f, backlog=0):
        try:
            ...
            current_data = {
                ...
                "is_stopping": getattr(self, "stop_requested", False) or os.path.exists(STOP_FILE)
            }
            tmp_file = os.path.join(BASE_DIR, f".progress.json.tmp.{os.getpid()}")
