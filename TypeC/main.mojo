...
def run_pipeline(start_row: Int, mode: String, sheet_id: String, apply_formatting: Bool) raises:
    ...
    print("🚀 Mojo: Starting Type C Pipeline (Modern Engine)...")
...
async def corrected_run_typec(start_row, mode, sheet_id=None, apply_formatting=True):
                        ...
                        async with semaphore:
                            ...
                            async with session.get(url, timeout=15) as resp:
                                ...
                                async with session.post(f"{CONFIG['GEMINI_API_URL']}?key={CONFIG['GEMINI_API_KEY']}", json={"contents":[{"parts":[{"text":prompt}]}]}) as g_resp:
                                    ...
                                    await ws.update(f"H{r_idx}:N{r_idx}", [["Yes", sd, ld, f_id, "Done", "Done", "Done"]], value_input_option='USER_ENTERED')
                                    successful_count += 1
                    except: pass
                    finally:
                        current_count += 1
...
"""
    ...
    var corrected_func = globals["corrected_run_typec"]
    _ = asyncio.run(corrected_func(start_row, mode, sheet_id, apply_formatting))
    print("✅ Mojo: Type C Pipeline Completed.")
